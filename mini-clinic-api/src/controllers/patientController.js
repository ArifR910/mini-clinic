...
const getPatientById = async (req, res) => {
    ...
    try {
        ...
        return sendSuccess(res, 200, 'Berhasil mengambil detail pasien', patients[0]);
    } catch (error) {
...
const createPatient = async (req, res) => {
    ...
    if (!nik || !name || !birth_date || !gender) {
        return sendError(res, 400, 'NIK, Nama, Tanggal Lahir, dan Jenis Kelamin wajib diisi!');
    }

    try {
        const [existing] = await db.query('SELECT id FROM patients WHERE nik = ?', [nik]);
        ...
        const mr_number = `RM-${year}-${nextNumber}`;

        const [result] = await db.query(
            `INSERT INTO patients (mr_number, nik, name, birth_date, gender, address, phone) 
             VALUES (?, ?, ?, ?, ?, ?, ?)`,
            [mr_number, nik, name, birth_date, gender, address || null, phone || null]
        );
...
const updatePatient = async (req, res) => {
    ...
    try {
        ...
        return sendError(res, 500, 'Terjadi kesalahan saat memperbarui data pasien');
    }
};
